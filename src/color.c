...
/*   Updated: 2025/06/26 14:28:13 by anavas-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	get_object_color(t_global *global)
{
	...
	if (global->c_ray.hit.obj_type == 0)
		return (global->scene.spheres[global->c_ray.hit.obj_index].material.color);
	else if (global->c_ray.hit.obj_type == 1)
		return (global->scene.planes[global->c_ray.hit.obj_index].material.color);
	else if (global->c_ray.hit.obj_type == 2)
		return (global->scene.cyls[global->c_ray.hit.obj_index].material.color);
	return ((t_color){0, 0, 0});
}
