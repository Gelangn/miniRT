		...
		float fresnel, float n1, float n2, t_ray_result *next, int count)
{
	...
	if (global->c_ray.hit.obj_type == 2)
		// Si es un cilindro (valor correcto)
...
int	generate_secondary_rays(t_global *global, t_ray_result *next)
{
	...
	count = 0;
	prepare_material_data(global, &transparency, &reflectivity, &fresnel, &n1,
			&n2);
	...
	count = add_refraction_ray(global, transparency, fresnel, n1, n2, next,
			count);
	return (count);
}
		...
		t_vector direction, int max_depth)
{
	...
	while (depth < real_max_depth && curr_count > 0)
	{
		...
		next_count = prepare_next_level(global, current_level, curr_count,
				next_level);
