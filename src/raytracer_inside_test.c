...
/*   Updated: 2025/06/21 14:18:27 by anavas-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_inside_cylinder(t_cylinder *cyl, t_vector point)
{
	...
	proj_on_axis = multiply(axis_dir, proj_length);
...
int	is_inside_object(t_global *global, t_intersec isec, t_vector origin)
{
	...
	if (isec.obj_type == 0)
	{
		return (is_inside_sphere(&global->scene.spheres[isec.obj_index],
				origin));
	}
	// Cilindros
	else if (isec.obj_type == 2)
	{
		return (is_inside_cylinder(&global->scene.cyls[isec.obj_index],
				origin));
	}
