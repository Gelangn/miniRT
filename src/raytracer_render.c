...
/*   Updated: 2025/06/27 10:25:38 by anavas-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	precal_camera_axis(t_global *global)
{
	...
	cam->fw_axis = norm(cam->dir);
	cam->right_axis = norm(cross((t_vector){0, 1, 0}, cam->fw_axis));
	cam->up_axis = norm(cross(cam->fw_axis, cam->right_axis));
	if (cam->roll_angle != 0)
	{
