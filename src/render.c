	...
	while (++x < (WIN_W - MARGIN))
	{
		...
		while (++y < (WIN_H - MARGIN))
		{
			if (x == WIN_W/2 && y == WIN_H/2) {
				t_vector ray_dir = get_ray_direction(global->scene.cam, x, y);
				printf("Center ray direction: (%f, %f, %f)\n", ray_dir.x, ray_dir.y, ray_dir.z);
				t_intersec center_intersec = find_closest_intersec(global, global->scene.cam.pos, ray_dir);
				printf("Center ray hit: type=%d, index=%d, distance=%f\n", 
					   center_intersec.obj_type, center_intersec.obj_index, center_intersec.dist);
			}
...
void	render(t_global *global)
{
	...
	if (comp_floats(magnitude(global->scene.cam.orientation), 0))
	{
		...
		global->scene.cam.orientation = subtract((t_vector){0,0,0}, global->scene.cam.pos);
	...
	printf("Camera position: (%f, %f, %f)\n", 
		   global->scene.cam.pos.x, global->scene.cam.pos.y, global->scene.cam.pos.z);
	printf("Camera orientation: (%f, %f, %f)\n", 
		   global->scene.cam.orientation.x, global->scene.cam.orientation.y, global->scene.cam.orientation.z);
